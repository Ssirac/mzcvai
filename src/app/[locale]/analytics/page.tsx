...
function Table({ title, rows, labelKey }: { title: string; rows: Row[]; labelKey: "name" | "key" }) {
  ...
  return (
    <div className="card p-4 sm:p-5">
      ...
      {rows.length === 0 ? (
        <div className="text-ink-3 text-xs py-4 text-center">{t("tableNoData")}</div>
      ) : (
...
export default function AnalyticsPage() {
  ...
  const funnelStages = f
    ? [
        { label: t("fSent"), value: f.sent, cls: "bg-line-strong" },
        { label: t("fDelivered"), value: f.delivered, cls: "bg-blue-500" },
        { label: t("fOpened"), value: f.opened, cls: "bg-violet-500" },
        { label: t("fReply"), value: f.replied, cls: "bg-accent", star: true },
      ]
    : [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5">
        ...
        {loading ? (
          ...
          <div className="card p-10 text-center">
            ...
            <div className="text-ink font-semibold mb-1">{t("noStats")}</div>
            <div className="text-ink-3 text-sm max-w-sm mx-auto">{t("noStatsHint")}</div>
          </div>
        ) : (
          <>
            ...
            <div className="card card-lift p-5 sm:p-6 grid gap-6 lg:grid-cols-[minmax(0,280px)_1fr] lg:items-center">
              <div>
                <div className="eyebrow mb-2">{t("replyRate")}</div>
                ...
                <p className="text-sm text-ink-2 mt-3 leading-relaxed">
                  {t("totalReplies", { replied: f.replied, sent: f.sent })}
                </p>
                {data && data.last30.sent > 0 && (
                  <p className="text-xs text-ink-3 mt-1.5">
                    {t("last30")}: <span className="tabular text-ink-2">{data.last30.replyRate}%</span> ({data.last30.replied}/{data.last30.sent})
                  </p>
                )}
              </div>
              <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <Stat label={t("delivered")} value={`${f.deliveryRate}%`} sub={`${f.delivered} ${t("mail")}`} color="text-blue-400" />
              <Stat label={t("openRate")} value={`${f.openRate}%`} sub={`${f.opened} ${t("mail")}`} color="text-violet-400" />
              <Stat label={t("bounced")} value={`${f.bounceRate}%`} sub={`${f.bounced} ${t("bounce")}`} color="text-red-400" />
              <Stat label={t("followup")} value={f.followUps} sub={t("sent")} color="text-blue-400" />
            </div>

            {/* Placement pipeline */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <Stat label={t("interview")} value={data?.pipeline?.interviews ?? 0} color="text-violet-400" sub={t("activeStage")} />
              <Stat label={t("placed")} value={data?.pipeline?.placed ?? 0} color="text-emerald-400" sub={t("placement")} />
              <Stat label={t("last30Rate")} value={`${data?.last30.replyRate ?? 0}%`} sub={`${data?.last30.replied ?? 0}/${data?.last30.sent ?? 0}`} color="text-green-400" />
            </div>
